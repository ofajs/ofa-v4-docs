...
ルートのプロパティを使用して要素のルートノードを取得します。

画面上では、一般的な要素のルートノードは [document](https://developer.mozilla.org/en-US/docs/Web/API/Document) インスタンスです。
...
げんいんコンポーネントの内 げんそうは そとげんきょうから かくりつして できており、シャドウノードのうちの もとぞくせいは シャドウコンポーネントです。
