...
```markdown
`one` 메서드는 이벤트 처리 함수를 바인딩하는 데 사용되지만, 해당 처리 함수는 이벤트가 처음 발생할 때에만 한 번 실행됩니다. 한 번 실행된 후에는 이벤트 처리 함수가 자동으로 제거되어 중복 트리거가 방지됩니다.
```
...
上面의 예제에서 "Click Me" 버튼을 클릭하면 "Button Clicked!"가 표시되지만 "Inner Div Clicked!" 및 "Outer Div Clicked!" 이벤트는 트리거되지 않습니다. 이는 버튼의 클릭 이벤트 핸들러에서 `event.stopPropagation()`을 호출하여 이벤트 전파를 차단했기 때문입니다.
